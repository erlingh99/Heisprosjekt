...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    if (heli_q8_B.RateTransitiony > heli_q8_P.DeadZoney_End) {
      /* Sum: '<S2>/Sum' */
      rtb_Sum = heli_q8_B.RateTransitiony - heli_q8_P.DeadZoney_End;
    } else if (heli_q8_B.RateTransitiony >= heli_q8_P.DeadZoney_Start) {
      /* Sum: '<S2>/Sum' */
      rtb_Sum = 0.0;
    } else {
      /* Sum: '<S2>/Sum' */
      rtb_Sum = heli_q8_B.RateTransitiony - heli_q8_P.DeadZoney_Start;
    }
     ...
     *  Gain: '<S4>/Gain: y'
     */
    heli_q8_B.Joystick_gain_y = heli_q8_P.Gainy_Gain * rtb_Sum *
      heli_q8_P.Joystick_gain_y;

    /* SignalConversion generated from: '<S5>/MatrixMultiply1' */
    rtb_TmpSignalConversionAtMatr_0 = heli_q8_B.Joystick_gain_x;
    rtb_TmpSignalConversionAtMatr_1 = heli_q8_B.Joystick_gain_y;

    /* Product: '<S5>/MatrixMultiply1' incorporates:
     *  Constant: '<S5>/Constant2'
     *  SignalConversion generated from: '<S5>/MatrixMultiply1'
     */
    heli_q8_B.MatrixMultiply1[0] = 0.0;
    heli_q8_B.MatrixMultiply1[0] += heli_q8_P.F[0] * heli_q8_B.Joystick_gain_x;
    heli_q8_B.MatrixMultiply1[0] += heli_q8_P.F[2] * heli_q8_B.Joystick_gain_y;
    heli_q8_B.MatrixMultiply1[1] = 0.0;
    heli_q8_B.MatrixMultiply1[1] += heli_q8_P.F[1] * heli_q8_B.Joystick_gain_x;
    heli_q8_B.MatrixMultiply1[1] += heli_q8_P.F[3] * heli_q8_B.Joystick_gain_y;
  }
  ...
  tmp[3] = heli_q8_B.Integrator;
  tmp[4] = heli_q8_B.Integrator1;
  for (i = 0; i < 2; i++) {
    ...
    rtb_Backgain = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      rtb_Backgain += heli_q8_P.K[(i_0 << 1) + i] * tmp[i_0];
    }

    heli_q8_B.Sum1[i] = heli_q8_B.MatrixMultiply1[i] - rtb_Backgain;
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    heli_q8_B.TmpSignalConversionAtToFileInpo[0] = heli_q8_B.Joystick_gain_x;
    heli_q8_B.TmpSignalConversionAtToFileInpo[1] = heli_q8_B.Joystick_gain_y;
    heli_q8_B.TmpSignalConversionAtToFileInpo[2] = heli_q8_B.PitchCounttorad;
    heli_q8_B.TmpSignalConversionAtToFileInpo[3] =
      heli_q8_B.ElevationTransferFcn;
  }

  for (i = 0; i < 5; i++) {
    /* Integrator: '<S7>/Integrator' */
    heli_q8_B.Integrator_i[i] = heli_q8_X.Integrator_CSTATE_m[i];
  }
      ...
      if (rtmIsMajorTimeStep(heli_q8_M) ) {
        {
          if (!(++heli_q8_DW.ToFile_IWORK.Decimation % 1) &&
              (heli_q8_DW.ToFile_IWORK.Count * (4 + 1)) + 1 < 100000000 ) {
            ...
            if (fp != (NULL)) {
              ...
              u[4] = heli_q8_B.TmpSignalConversionAtToFileInpo[3];
              if (fwrite(u, sizeof(real_T), 4 + 1, fp) != 4 + 1) {
                rtmSetErrorStatus(heli_q8_M,
                                  "Error writing to MAT-file ref_vs_measures.mat");
                return;
              }

              if (((++heli_q8_DW.ToFile_IWORK.Count) * (4 + 1))+1 >= 100000000)
              {
                (void)fprintf(stdout,
                              "*** The ToFile block will stop logging data before\n"
                              "    the simulation has ended, because it has reached\n"
                              "    the maximum number of elements (100000000)\n"
                              "    allowed in MAT-file ref_vs_measures.mat.\n");
              }
            }
          }
        }
   ...
   */
  rtb_Backgain = heli_q8_B.Sum1[0] + heli_q8_P.vs_0;

  /* Sum: '<S1>/Add' */
  rtb_Frontgain = rtb_Backgain - heli_q8_B.Sum1[1];

  /* Sum: '<S1>/Subtract' */
  rtb_Backgain += heli_q8_B.Sum1[1];

  /* Gain: '<S1>/Back gain' */
  rtb_Backgain *= heli_q8_P.Backgain_Gain;
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* Sum: '<S5>/Sum' */
    heli_q8_B.Sum_h = heli_q8_B.PitchCounttorad - heli_q8_B.Joystick_gain_x;
    for (i = 0; i < 5; i++) {
      /* Product: '<S7>/MatrixMultiply' incorporates:
       *  Constant: '<S7>/Constant1'
       */
      heli_q8_B.MatrixMultiply[i] = 0.0;
      heli_q8_B.MatrixMultiply[i] += heli_q8_P.G[i] *
        rtb_TmpSignalConversionAtMatr_0;
      heli_q8_B.MatrixMultiply[i] += heli_q8_P.G[i + 5] *
        rtb_TmpSignalConversionAtMatr_1;
    }

    /* Product: '<S7>/MatrixMultiply1' incorporates:
     *  Constant: '<S7>/Constant2'
     */
    heli_q8_B.MatrixMultiply1_m[0] = 0.0;
    heli_q8_B.MatrixMultiply1_m[0] += heli_q8_P.F[0] *
      rtb_TmpSignalConversionAtMatr_0;
    heli_q8_B.MatrixMultiply1_m[0] += heli_q8_P.F[2] *
      rtb_TmpSignalConversionAtMatr_1;
    heli_q8_B.MatrixMultiply1_m[1] = 0.0;
    heli_q8_B.MatrixMultiply1_m[1] += heli_q8_P.F[1] *
      rtb_TmpSignalConversionAtMatr_0;
    heli_q8_B.MatrixMultiply1_m[1] += heli_q8_P.F[3] *
      rtb_TmpSignalConversionAtMatr_1;
  }
  ...
  for (i = 0; i < 2; i++) {
    rtb_Backgain = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      rtb_Backgain += heli_q8_P.K[(i_0 << 1) + i] * heli_q8_B.Integrator_i[i_0];
    }

    tmp_0[i] = heli_q8_B.MatrixMultiply1_m[i] - rtb_Backgain;
  }
  ...
  for (i = 0; i < 5; i++) {
     ...
     *  Product: '<S7>/MatrixMultiply4'
     */
    rtb_Backgain = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      rtb_Backgain += heli_q8_P.A[5 * i_0 + i] * heli_q8_B.Integrator_i[i_0];
    }

    /* Sum: '<S7>/Sum2' incorporates:
     *  Constant: '<S7>/Constant4'
     *  Constant: '<S7>/Constant5'
     *  Product: '<S7>/MatrixMultiply3'
     *  Product: '<S7>/MatrixMultiply4'
     */
    heli_q8_B.Sum2_p[i] = (rtb_Backgain + heli_q8_B.MatrixMultiply[i]) +
      (heli_q8_P.B[i + 5] * tmp_0[1] + heli_q8_P.B[i] * tmp_0[0]);
  }
}
...
void heli_q8_derivatives(void)
{
  ...
  _rtXdot->Integrator_CSTATE = heli_q8_B.Sum_h;
  ...
  for (i = 0; i < 5; i++) {
    _rtXdot->Integrator_CSTATE_m[i] = heli_q8_B.Sum2_p[i];
  }
...
void heli_q8_initialize(void)
{
  ...
  {
    FILE *fp = (NULL);
    char fileName[509] = "ref_vs_measures.mat";
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error creating .mat file ref_vs_measures.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp, 4 + 1, 0, "ans")) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error writing mat file header to file ref_vs_measures.mat");
      return;
    }
...

  {
    ...
    for (i = 0; i < 5; i++) {
      heli_q8_X.Integrator_CSTATE_m[i] = heli_q8_P.Integrator_IC_k;
    }

    /* End of InitializeConditions for Integrator: '<S7>/Integrator' */

    /* InitializeConditions for Integrator: '<S8>/Integrator' */
    heli_q8_X.Integrator_CSTATE_n = heli_q8_P.Integrator_IC_ky;
  }
}
...
void heli_q8_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      ...
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(heli_q8_M,
                          "Error closing MAT-file ref_vs_measures.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(heli_q8_M,
                          "Error reopening MAT-file ref_vs_measures.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 4 + 1, heli_q8_DW.ToFile_IWORK.Count, "ans"))
      {
        rtmSetErrorStatus(heli_q8_M,
                          "Error writing header for ans to MAT-file ref_vs_measures.mat");
      }
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (3775226793U);
  heli_q8_M->Sizes.checksums[1] = (3192274425U);
  heli_q8_M->Sizes.checksums[2] = (3310147384U);
  heli_q8_M->Sizes.checksums[3] = (4116127766U);
...

  {
    ...
    heli_q8_B.TmpSignalConversionAtToFileInpo[3] = 0.0;
    heli_q8_B.K_ei = 0.0;
  ...
  heli_q8_M->Sizes.numBlockPrms = (204);/* Sum of parameter "widths" */
  return heli_q8_M;
}
