...
typedef RT_MODEL_IMU_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * IMU(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(IMU_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  {
    stream_shutdown(IMU_DW.StreamCall1_Stream);
  }
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://IMU:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: IMU -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
