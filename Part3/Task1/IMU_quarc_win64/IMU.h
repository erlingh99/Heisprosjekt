...
#define IMU_rtModel                    RT_MODEL_IMU_T

/* Block signals (default storage) */
typedef struct {
  real_T Switch[10];                   /* '<S1>/Switch' */
  real_T Gain1[3];                     /* '<S1>/Gain1' */
  real_T Gain2[3];                     /* '<S1>/Gain2' */
  uint8_T StreamCall1_o2;              /* '<S1>/Stream Call1' */
} B_IMU_T;

/* Block states (default storage) for system '<Root>' */
typedef struct {
  real_T Memory_PreviousInput[10];     /* '<S1>/Memory' */
  t_stream StreamCall1_Stream;         /* '<S1>/Stream Call1' */
  struct {
    void *LoggedData;
  ...
  struct {
    void *LoggedData;
  } Connected_PWORK;                   /* '<S1>/Connected' */

  uint8_T StreamCall1_State;           /* '<S1>/Stream Call1' */
} DW_IMU_T;

/* Backward compatible GRT Identifiers */
#define rtB                            IMU_B
#define BlockIO                        B_IMU_T
#define rtP                            IMU_P
#define Parameters                     P_IMU_T
#define rtDWork                        IMU_DW
#define D_Work                         DW_IMU_T
