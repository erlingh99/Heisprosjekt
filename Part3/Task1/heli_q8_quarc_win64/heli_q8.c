...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* S-Function Block: heli_q8/IMU1/Stream Call1 (stream_call_block) */
    {
      ...
      rtb_StreamCall1_o2 = heli_q8_DW.StreamCall1_State;
     ...
     */
    rtb_p += heli_q8_P.p_bias_Value;

    /* Sum: '<Root>/Add2' incorporates:
     *  Constant: '<Root>/e_bias'
     */
    rtb_e += heli_q8_P.e_bias_Value;
   ...
   */
  rtb_p_0[0] = heli_q8_X.Integrator_CSTATE[0];
  rtb_p_0[1] = heli_q8_X.Integrator_CSTATE[1];
  rtb_p_0[2] = heli_q8_X.Integrator_CSTATE[3];
  rtb_p_0[3] = heli_q8_X.Integrator_CSTATE_d;
  rtb_p_0[4] = heli_q8_X.Integrator1_CSTATE;
  ...
  for (i = 0; i < 2; i++) {
    rtb_Backgain = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      rtb_Backgain += heli_q8_P.K[(i_0 << 1) + i] * rtb_p_0[i_0];
    }

    rtb_Sum1[i] = heli_q8_B.Multiply[i] - rtb_Backgain;
  }
   ...
   */
  rtb_Backgain = heli_q8_P.vs_0 + rtb_Sum1[0];
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* Gain: '<S4>/Gain1' */
    for (i = 0; i < 3; i++) {
      tmp_2[i] = heli_q8_P.Gain1_Gain[i + 6] * heli_q8_B.Switch[5] +
    ...
    for (i = 0; i < 5; i++) {
      ...
      for (i_0 = 0; i_0 < 5; i_0++) {
        heli_q8_B.Gain[i] += heli_q8_P.C[5 * i_0 + i] * rtb_p_0[i_0];
      }
   ...
   *  Integrator: '<S8>/Integrator'
   */
  heli_q8_B.Sum = heli_q8_X.Integrator_CSTATE[0] - heli_q8_B.Joystick_gain_x;

  /* Sum: '<S6>/Sum2' incorporates:
   *  Integrator: '<S8>/Integrator'
   */
  heli_q8_B.Sum2 = heli_q8_X.Integrator_CSTATE[3] - heli_q8_B.Joystick_gain_y;
  for (i = 0; i < 5; i++) {
    ...
    rtb_Backgain = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      rtb_Backgain += heli_q8_P.C[5 * i_0 + i] * heli_q8_X.Integrator_CSTATE[i_0];
    }

    rtb_p_0[i] = heli_q8_B.Gain[i] - rtb_Backgain;

    /* End of Sum: '<S8>/Sum' */

    /* Gain: '<S8>/Multiply1' */
    tmp[i] = heli_q8_P.B_est[i + 5] * rtb_Sum1[1] + heli_q8_P.B_est[i] *
      rtb_Sum1[0];
  }
  ...
  for (i = 0; i < 5; i++) {
    ...
    for (i_0 = 0; i_0 < 5; i_0++) {
       ...
       *  Integrator: '<S8>/Integrator'
       */
      rtb_Backgain += heli_q8_P.A_est[tmp_3] * heli_q8_X.Integrator_CSTATE[i_0];

      /* Gain: '<S8>/Multiply2' */
      tmp_0[i] += heli_q8_P.L[tmp_3] * rtb_p_0[i_0];
    }

    /* Sum: '<S8>/Sum2' incorporates:
     *  Gain: '<S8>/Multiply'
     */
    heli_q8_B.Sum2_b[i] = (tmp[i] + tmp_0[i]) + rtb_Backgain;
  }
}
...
void heli_q8_derivatives(void)
{
  ...
  for (i = 0; i < 5; i++) {
    _rtXdot->Integrator_CSTATE[i] = heli_q8_B.Sum2_b[i];
  }

  /* End of Derivatives for Integrator: '<S8>/Integrator' */

  /* Derivatives for Integrator: '<S6>/Integrator' */
  _rtXdot->Integrator_CSTATE_d = heli_q8_B.Sum;
...
void heli_q8_initialize(void)
{
...

  {
    ...
    heli_q8_X.Integrator_CSTATE_d = heli_q8_P.Integrator_IC_g;
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  heli_q8_M->Sizes.numBlockIO = (15);  /* Number of block outputs */
  heli_q8_M->Sizes.numBlockPrms = (533);/* Sum of parameter "widths" */
  return heli_q8_M;
}
