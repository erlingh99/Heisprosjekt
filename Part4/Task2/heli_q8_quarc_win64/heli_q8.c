 ...
 *
 * Model version              : 11.282
 * Simulink Coder version : 9.4 (R2020b) 29-Jul-2020
 * C source code generated on : Wed Nov 17 13:39:47 2021
 *
...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    if (heli_q8_B.Gain2[2] == 0.0) {
      ...
      heli_q8_B.p = rtb_e * 3.1415926535897931 / 2.0;
    } else {
      /* '<S9>:1:7' */
      heli_q8_B.p = heli_q8_B.Gain2[1] / heli_q8_B.Gain2[2];
      heli_q8_B.p = atan(heli_q8_B.p);
    }
    ...
    /* '<S3>:1:8' */
    /* '<S3>:1:11' */
    rtb_e = tan(heli_q8_B.Sum2);
    rtb_Frontgain = sin(heli_q8_B.p);
    euler_rates_tmp = cos(heli_q8_B.p);
    euler_rates_tmp_0 = cos(heli_q8_B.Sum2);
    tmp_0[0] = 1.0;
    tmp_0[3] = rtb_Frontgain * rtb_e;
    tmp_0[6] = euler_rates_tmp * rtb_e;
    tmp_0[1] = 0.0;
    tmp_0[4] = euler_rates_tmp;
    tmp_0[7] = -rtb_Frontgain;
    tmp_0[2] = 0.0;
    tmp_0[5] = rtb_Frontgain / euler_rates_tmp_0;
    ...
    if (rtb_e != 0.0) {
      ...
      for (iy = 0; iy < 6; iy++) {
        for (jBcol = 0; jBcol < 7; jBcol++) {
          A_tmp_tmp = 6 * jBcol + iy;
          A_tmp[jBcol + 7 * iy] = heli_q8_P.C_d[A_tmp_tmp];
          X[A_tmp_tmp] = 0.0;
          for (i = 0; i < 7; i++) {
            X[A_tmp_tmp] += heli_q8_P.C_d[6 * i + iy] *
              heli_q8_DW.UnitDelay1_DSTATE[7 * jBcol + i];
          }
        }
      }
      ...
      for (iy = 0; iy < 6; iy++) {
        for (jBcol = 0; jBcol < 6; jBcol++) {
          rtb_e = 0.0;
          for (i = 0; i < 7; i++) {
            rtb_e += X[6 * i + iy] * A_tmp[7 * jBcol + i];
          }
      ...
      for (i = 0; i < 5; i++) {
        ...
        for (kBcol = 2; kBcol <= 6 - i; kBcol++) {
          ix++;
          rtb_Frontgain = fabs(A[ix]);
          if (rtb_Frontgain > rtb_e) {
            iy = kBcol - 1;
            rtb_e = rtb_Frontgain;
          }
        }
      ...
      for (iy = 0; iy < 6; iy++) {
        for (jBcol = 0; jBcol < 7; jBcol++) {
          A_tmp_tmp = jBcol + 7 * iy;
          X[A_tmp_tmp] = 0.0;
          for (i = 0; i < 7; i++) {
            X[A_tmp_tmp] += heli_q8_DW.UnitDelay1_DSTATE[7 * i + jBcol] * A_tmp
              [7 * iy + i];
          }
        }
      }
      ...
      tmp_2[2] = heli_q8_B.Sum2;
      tmp_2[3] = heli_q8_B.euler_rates[1];
      tmp_2[4] = heli_q8_B.TravelCounttorad;
      tmp_2[5] = heli_q8_B.euler_rates[2];
      for (iy = 0; iy < 6; iy++) {
        tmp_3[iy] = 0.0;
        for (jBcol = 0; jBcol < 7; jBcol++) {
          tmp_3[iy] += heli_q8_P.C_d[6 * jBcol + iy] * heli_q8_B.UnitDelay[jBcol];
        }

        tmp_4[iy] = tmp_2[iy] - tmp_3[iy];
      }

      for (iy = 0; iy < 7; iy++) {
        rtb_e = 0.0;
        for (jBcol = 0; jBcol < 6; jBcol++) {
          rtb_e += X[7 * jBcol + iy] * tmp_4[jBcol];
        }
      ...
      for (i = 0; i < 7; i++) {
        ...
        for (iy = 0; iy < 7; iy++) {
          A_tmp_tmp = i + 7 * iy;
          P_hat_tmp[A_tmp_tmp] = 0.0;
          for (jBcol = 0; jBcol < 6; jBcol++) {
            P_hat_tmp[A_tmp_tmp] += X[7 * jBcol + i] * heli_q8_P.C_d[6 * iy +
              jBcol];
          }
        }
      }
      ...
      for (iy = 0; iy < 49; iy++) {
        c_I_0[iy] = (real_T)c_I[iy] - P_hat_tmp[iy];
      }

      for (iy = 0; iy < 7; iy++) {
        for (jBcol = 0; jBcol < 7; jBcol++) {
          A_tmp_tmp = iy + 7 * jBcol;
          c_I_1[A_tmp_tmp] = 0.0;
          for (i = 0; i < 7; i++) {
            c_I_1[A_tmp_tmp] += c_I_0[7 * i + iy] *
              heli_q8_DW.UnitDelay1_DSTATE[7 * jBcol + i];
          }

          b_I_0[jBcol + 7 * iy] = (real_T)b_I[A_tmp_tmp] - P_hat_tmp[A_tmp_tmp];
        }
        ...
        for (jBcol = 0; jBcol < 6; jBcol++) {
          ...
          A_tmp[A_tmp_tmp] = 0.0;
          for (i = 0; i < 6; i++) {
            A_tmp[A_tmp_tmp] += X[7 * i + iy] * heli_q8_P.R_d[6 * jBcol + i];
          }
        }
      }
      ...
      for (iy = 0; iy < 7; iy++) {
        for (jBcol = 0; jBcol < 7; jBcol++) {
          ...
          c_I_0[A_tmp_tmp] = 0.0;
          for (i = 0; i < 7; i++) {
            c_I_0[A_tmp_tmp] += c_I_1[7 * i + iy] * b_I_0[7 * jBcol + i];
          }

          P_hat_tmp[A_tmp_tmp] = 0.0;
          for (i = 0; i < 6; i++) {
            P_hat_tmp[A_tmp_tmp] += A_tmp[7 * i + iy] * X[7 * i + jBcol];
          }
        }
      }

      for (iy = 0; iy < 49; iy++) {
        heli_q8_B.P_hat[iy] = c_I_0[iy] + P_hat_tmp[iy];
      }
    } else {
      /* '<S12>:1:8' */
      for (i = 0; i < 7; i++) {
      ...
      /* '<S12>:1:9' */
      memcpy(&heli_q8_B.P_hat[0], &heli_q8_DW.UnitDelay1_DSTATE[0], 49U * sizeof
             (real_T));
    }
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* '<S2>:1:4' */
    for (ix = 0; ix < 7; ix++) {
      heli_q8_B.y[ix] = heli_q8_B.P_hat[7 * ix + ix];
    }
  ...
  for (iy = 0; iy < 2; iy++) {
    ...
    for (jBcol = 0; jBcol < 5; jBcol++) {
      rtb_e += heli_q8_P.K[(jBcol << 1) + iy] * tmp[jBcol];
    }
   ...
   */
  rtb_e = heli_q8_P.vs_0 + heli_q8_B.Sum1[0];

  /* Sum: '<S1>/Add' */
  rtb_Frontgain = rtb_e - heli_q8_B.Sum1[1];

  /* Sum: '<S1>/Subtract' */
  rtb_e += heli_q8_B.Sum1[1];

  /* Gain: '<S1>/Back gain' */
  rtb_e *= heli_q8_P.Backgain_Gain;
  ...
  for (iy = 0; iy < 7; iy++) {
    ...
    for (jBcol = 0; jBcol < 7; jBcol++) {
      i = 7 * jBcol + iy;
      c_I_0[i] = 0.0;
      for (A_tmp_tmp = 0; A_tmp_tmp < 7; A_tmp_tmp++) {
        c_I_0[i] += heli_q8_P.A_d[7 * A_tmp_tmp + iy] * heli_q8_B.P_hat[7 *
          jBcol + A_tmp_tmp];
      }

      tmp_1[iy] += heli_q8_P.A_d[i] * heli_q8_B.x_hat[jBcol];
    }

    heli_q8_B.x_bar[iy] = tmp_1[iy] + (heli_q8_P.B_d[iy + 7] * heli_q8_B.Sum1[1]
      + heli_q8_P.B_d[iy] * heli_q8_B.Sum1[0]);
    for (jBcol = 0; jBcol < 7; jBcol++) {
      rtb_e = 0.0;
      for (i = 0; i < 7; i++) {
        rtb_e += c_I_0[7 * i + iy] * heli_q8_P.A_d[7 * i + jBcol];
      }
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* Sum: '<S8>/Sum2' */
    heli_q8_B.Sum2_l = heli_q8_B.x_hat[4] - heli_q8_B.Joystick_gain_y;
  }
}
...
void heli_q8_update0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    memcpy(&heli_q8_DW.UnitDelay1_DSTATE[0], &heli_q8_B.P_bar[0], 49U * sizeof
           (real_T));
  }
...
void heli_q8_initialize(void)
{
...

  {
    ...
    memcpy(&heli_q8_DW.UnitDelay1_DSTATE[0], &heli_q8_P.P_0[0], 49U * sizeof
           (real_T));
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (2501630278U);
  heli_q8_M->Sizes.checksums[1] = (1223902178U);
  heli_q8_M->Sizes.checksums[2] = (3159200490U);
  heli_q8_M->Sizes.checksums[3] = (1939376937U);
  ...
  heli_q8_M->Sizes.numBlocks = (99);   /* Number of blocks */
  heli_q8_M->Sizes.numBlockIO = (30);  /* Number of block outputs */
  heli_q8_M->Sizes.numBlockPrms = (695);/* Sum of parameter "widths" */
  return heli_q8_M;
}
